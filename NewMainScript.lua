...
local function downloadPremadeProfiles(commit)
    ...
    local success, response = pcall(function()
        return game:HttpGet('https://api.github.com/repos/Yanrxc/yan/contents/profiles/premade?ref=' .. commit)
    end)
            ...
            for _, file in pairs(files) do
                if file.name and file.name:find('.txt') and file.name ~= 'commit.txt' then
                    ...
                    if not isfile(filePath) then
                        local dl = file.download_url or ('https://raw.githubusercontent.com/Yanrxc/yan/' .. commit .. '/profiles/premade/' .. file.name)
...
if not shared.VapeDeveloper then
	local _, subbed = pcall(function()
		return game:HttpGet('https://github.com/Yanrxc/yan')
	end)
